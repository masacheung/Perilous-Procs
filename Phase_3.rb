...
# The method should replace the elements that return true when passed into the first proc with their return values
# when they are passed into the second proc. This method should mutate the input array and return nil.
...
# If an original word returns true for multiple procs, then the suffixes should be appended in the order
...
def proctition_platinum(array, *proc)
    ans = Hash.new
    proc.each.with_index {|ele, i| ans[i+1] = []}
    array.each do |ele|
        proc.each.with_index do |p, i|
            if p.call(ele)
                ans[i+1] << ele
            end
        end
...
# The hash contains procs as both keys and values. The method should return a new sentence where each word of
# the input sentence is changed by a value proc if the original word returns true when passed into the key proc.
# If an original word returns true for multiple key procs, then the value proc changes should be applied in the
# order that they appear in the hash.
...
# The hash contains procs as both keys and values. The method should return a new sentence where each word
# of the input sentence is changed by a value proc if the original word returns true when passed into
# the key proc. If an original word returns true for multiple key procs, then only the value proc that
